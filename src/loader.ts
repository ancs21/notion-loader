...
export function notionLoader({
  ...
  return {
    ...
    // Astro 6 tightened the Loader schema type to expect a Zod v4 $ZodType.
    // The notionPageSchema() factory returns a Zod v3-style ZodObject which is
    // structurally compatible at runtime but trips the new type. Cast through
    // `never` to satisfy the API contract; behavior unchanged.
    schema: (async () =>
      notionPageSchema({
